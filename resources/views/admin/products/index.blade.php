    ...
    <x-slot name="header">
        <div class="flex flex-row items-center justify-between w-full">
            ...
            <a href="{{ route('admin.products.create') }}"
                class="px-5 py-3 font-bold text-white bg-indigo-700 rounded-full">Add Product</a>
        </div>
    </x-slot>
                ...
                @forelse ($products as $product)
                    <div class="flex flex-row items-center justify-between item-card">
                        <div class="flex flex-row items-center gap-x-3">
                            ...
                            <div>
                                <h3 class="text-2xl font-bold text-indigo-950">{{ $product->name }}</h3>
                        ...
                        <div class="flex flex-row items-center gap-x-3">
                            ...
                            <form method="POST" action="{{ route('admin.products.destroy', $product) }}">
                                @csrf
                                @method('DELETE')
                                <button class="px-5 py-3 font-bold text-white bg-red-700 rounded-full">Delete</button>
                            </form>
                        </div>
                    </div>
                @empty
                <p>Products Not Yet!.</p>
                @endforelse
            </div>
        </div>
    </div>
